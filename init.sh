...
echo "# This file is generated by the init.sh script" >$GITIGN_OVER
if test -f ".gitignore"; then
    echo "## .gitignore" >>$GITIGN_OVER
    cat .gitignore >>$GITIGN_OVER
fi
if test -f ".tclb/gitignore"; then
    echo "## .tclb/gitignore" >>$GITIGN_OVER
    cat .tclb/gitignore >>$GITIGN_OVER
fi
comm <(gitover ls-files | sort) <(gittclb ls-files | sort) -13 | sed 's|^|/|' >>$GITIGN_OVER

gitover config --local core.excludesfile "$GITIGN_OVER"
